...
use autozig_parser::{AutoZigConfig, IncludeZigConfig};
...
/// 2. Generate extern "C" FFI bindings directly from Rust signatures (IDL-driven)
...
pub fn autozig(input: TokenStream) -> TokenStream {
    ...
    let output = if config.has_rust_signatures() || !config.rust_structs.is_empty() || !config.rust_enums.is_empty() || !config.rust_trait_impls.is_empty() {
...
fn generate_trait_implementations(config: &AutoZigConfig) -> proc_macro2::TokenStream {
    ...
    for trait_impl in &config.rust_trait_impls {
        ...
        for method in &trait_impl.methods {
            ...
            // Phase 2: For opaque types, always generate FFI call (ignore user's simplified body)
            // Phase 1: Use original method body if available (preserves user logic like Option wrapping)
            let should_generate_ffi_call = trait_impl.is_opaque || method.body.is_none();
            
            if !should_generate_ffi_call {
                // Phase 1: Use the original body with unsafe wrapper (for ZST with complex logic)
...
fn generate_constructor(
    ...
    let params: Vec<_> = constructor.sig.inputs.iter()
        .filter_map(|input| {
            if let syn::FnArg::Typed(pat_type) = input {
                Some(pat_type)
            } else {
...
/// Generate FFI declarations for Zig functions used in trait implementations (Phase 1 & 2)
...
pub fn include_zig(input: TokenStream) -> TokenStream {
    ...
    let output = if config.has_rust_signatures() || !config.rust_structs.is_empty() || !config.rust_enums.is_empty() {
...
fn generate_trait_implementations_for_include(config: &IncludeZigConfig) -> proc_macro2::TokenStream {
    let mut impls = Vec::new();
    let mod_name_str = config.get_unique_mod_name();
    let mod_name = syn::Ident::new(&mod_name_str, proc_macro2::Span::call_site());
