...
//!
//! This example demonstrates AutoZig's trait support with zero-sized types (ZST).
//! It shows how Zig functions can implement Rust traits with zero overhead.
...
fn main() {
    ...
    let calculators: Vec<Box<dyn Calculator>> = vec![
        Box::new(ZigCalculator),
        Box::new(ZigCalculator::default()),
    ];
    ...
    println!("-----------------------------");
    println!("ZigCalculator size: {} bytes (Zero-Sized Type!)", 
