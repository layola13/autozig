...
use syn::{
    ...
    Result as ParseResult, Signature, ItemStruct, ItemEnum, ItemImpl,
};
...
impl Parse for AutoZigConfig {
    fn parse(input: ParseStream) -> ParseResult<Self> {
        ...
        if parts.len() == 1 {
            ...
            // Parse Rust definitions (enums, structs, function signatures, and trait impls) from second part
            let (rust_enums, rust_structs, rust_signatures, rust_trait_impls) = parse_rust_definitions(parts[1])?;
            ...
            Err(syn::Error::new(
                input.span(),
                "autozig! macro parsing error"
            ))
        }
    }
}

/// Parse Rust definitions (enums, structs, function signatures, and trait impls) from a string
fn parse_rust_definitions(input: &str) -> ParseResult<(Vec<RustEnumDefinition>, Vec<RustStructDefinition>, Vec<RustFunctionSignature>, Vec<RustTraitImpl>)> {
    ...
    let input_normalized = input_normalized.split_whitespace().collect::<Vec<_>>().join(" ");
    
    // Check if input is already wrapped in braces (from include_zig! macro)
    let input_content = if input_normalized.trim().starts_with('{') && input_normalized.trim().ends_with('}') {
        // Remove outer braces
                    ...
                    for inner_item in &items {
                        if let syn::Item::Impl(item_impl) = inner_item {
                            ...
                            if let Some(mut trait_impl) = parse_trait_impl(item_impl.clone()) {
                                eprintln!("Parser:   -> Parsed as TRAIT impl for {}", trait_impl.target_type);
                                ...
                                if opaque_types.contains(&trait_impl.target_type) {
                                    ...
                                    trait_impl.is_zst = false; // Opaque types are not ZST
                                }
                                ...
                                if let Some(inherent_impl) = parse_inherent_impl(item_impl.clone(), &opaque_types) {
                                    eprintln!("Parser:   -> SUCCESS: Parsed inherent impl for {}", inherent_impl.target_type);
                    ...
                    for inner_item in items {
                        match inner_item {
                            ...
                            syn::Item::Struct(item_struct) => {
                                ...
                                if !trait_impl_types.contains(&struct_name) && !is_opaque_struct(&item_struct) {
...
fn extract_zig_function_call(block: &syn::Block) -> Option<String> {
    // Try to find any function call in the block that looks like a Zig function (starts with "zig_")
...
fn extract_zig_function_from_stmt(stmt: &syn::Stmt) -> Option<String> {
    match stmt {
        syn::Stmt::Expr(expr, _) => {
            extract_zig_function_from_expr(expr)
        }
...
fn extract_zig_function_from_expr(expr: &syn::Expr) -> Option<String> {
    match expr {
        ...
        syn::Expr::Let(let_expr) => {
            extract_zig_function_from_expr(&let_expr.expr)
        }
        _ => None,
    }
}
...
fn parse_inherent_impl(
    ...
    for impl_item in &item_impl.items {
        if let syn::ImplItem::Fn(method) = impl_item {
            ...
            let is_constructor_attr = has_attribute(&method.attrs, "constructor");
            let is_destructor_attr = has_attribute(&method.attrs, "destructor");
            eprintln!("Parser: parse_inherent_impl:     constructor={}, destructor={}", is_constructor_attr, is_destructor_attr);
            
            if is_constructor_attr || is_destructor_attr {
                // Extract Zig function name from method body
                eprintln!("Parser: parse_inherent_impl:     Extracting zig function...");
                if let Some(zig_function) = extract_zig_function_call(&method.block) {
                    eprintln!("Parser: parse_inherent_impl:     Found zig function: {}", zig_function);
                    let trait_method = TraitMethod {
                        name: method.sig.ident.to_string(),
                        sig: method.sig.clone(),
                        zig_function,
...
impl Parse for IncludeZigConfig {
    fn parse(input: ParseStream) -> ParseResult<Self> {
        ...
        if input.peek(syn::Token![,]) {
            ...
            let (rust_enums, rust_structs, rust_signatures, rust_trait_impls) = parse_rust_definitions(&token_str)?;
...
impl IncludeZigConfig {
    ...
    pub fn get_unique_mod_name(&self) -> String {
        ...
        let path_without_ext = self.file_path
